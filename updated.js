...
app.post('/updated-algolia', async (req, res) => {
    try {
        // Get the query parameters from the request body or query string
        const { query, page, hitsPerPage, role, company_team_size, company_waas_stage, company_parent_sector, remote, min_experience, company_highlight_latinx, locations_for_search, job_type } = req.body;

        // Define the search parameters dynamically
        const searchParams = {
            query: query || "",
        ...
        if (remote !== undefined) filters.push(`remote:${remote}`);
        if (min_experience !== undefined) filters.push(`min_experience:${min_experience}`);
